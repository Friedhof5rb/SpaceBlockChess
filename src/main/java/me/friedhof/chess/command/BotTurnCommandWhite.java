...
public class BotTurnCommandWhite {

    public static void register (CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment) {
        dispatcher.register(CommandManager.literal("chess").then(CommandManager.literal("botTurn").then(CommandManager.literal("white").executes(BotTurnCommandWhite::run))));
    }
    ...
    private static int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        Bot bot = new Bot("white",w,current);
        context.getSource().getPlayer().sendMessage(Text.literal("White Bot started Calculating..."));
        Chess.setStart_time();
        TreeNode t = bot.computeBestMove();
        Chess.setEnd_time();
        context.getSource().getPlayer().sendMessage(Text.literal("White Bot finished Calculating. Now playing."));
        ...
        context.getSource().getPlayer().sendMessage(Text.literal("White Bot played it's move."));
        Chess.printDuration();
        return 1;
    }
